...
from dataset.datasets_portraitseg import PortraitSeg


def get_train_loader(cfg):
    dataset = Human(cfg, is_train=True)
    # each process load a subset of the original dataset exclusive to it
    train_sampler = data.DistributedSampler(dataset) if cfg.distributed else None 
...
def get_valid_loader(cfg):
    dataset = Human(cfg, is_train=False)
    valid_sampler = data.DistributedSampler(dataset) if cfg.distributed else None
...
class Human(data.Dataset):  
    def __init__(self, exp_args, is_train):
        assert exp_args.task in ['seg'], 'Error!, <task> should in [seg]'
        
        exp_args.istrain = is_train
